void drawForStateOrbitTime() {
  ...
  int i,j,n=0;
  textSize(20);
  text("Barchart to Display the Number of Days Each Kepler Planet Takes to Orbit it's Star", width/2, 50);
  ...
  for(j=0;j<600;j=j+20)
  {
    ...
    n=height-j;
    text(n/2,20,j);//Displays number beside long lines
    textSize(8);
  }
  
  for(TableRow row: table.rows())
  {
    float barHeight = row.getFloat("koi_orbital_period_days")*2 ;// Sets barHeight val to current float in data and multiplies by 2 to represent size on graph better
